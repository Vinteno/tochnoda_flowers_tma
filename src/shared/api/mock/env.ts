...
import { getPlatform } from '../../lib/bridge'
...
if (import.meta.env.DEV) {
  ...
  if (platform === 'max') {
    if (typeof window !== 'undefined') {
      const user = { id: 1, first_name: 'Max Mock' }
      const initDataStr = `user=${encodeURIComponent(JSON.stringify(user))}`

      ;(window as any).WebApp = {
        initData: initDataStr,
        initDataUnsafe: { user, start_param: '' },
        platform: 'max',
        ready: () => console.info('Mock MAX ready'),
        close: () => console.info('Mock MAX close'),
        BackButton: {
          show: () => console.info('Mock MAX BackButton show'),
          hide: () => console.info('Mock MAX BackButton hide'),
          onClick: () => console.info('Mock MAX BackButton onClick registered'),
          offClick: () => console.info('Mock MAX BackButton offClick registered'),
        },
